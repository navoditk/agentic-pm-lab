"""Evaluate progress.yaml against the current repo state and regenerate
PROGRESS.md's status table between the <!-- PROGRESS:START/END --> markers
(PLAN.md §6). Run by .github/workflows/progress-tracker.yml on every push to
...
PROGRESS_YAML_PATH = REPO_ROOT / "progress.yaml"
...
def build_day_table(progress: dict) -> str:
    ...
    for key in sorted(progress, key=lambda k: int(k.split("_")[1])):
        ...
        entry = progress[key]
        label = entry["label"]
        is_optional = int(day_num) in (13, 14)
        focus = f"*(optional)* {label}" if is_optional else label
        checks = entry.get("checks") or []
        note = "Optional AWS extension" if is_optional else ""
        if not checks:
            status = "⬜ Not started"
        else:
