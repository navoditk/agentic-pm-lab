...
lands somewhere plausible and assumes the section moved. Five such anchors
were live in this repository when this check was written, three of them in
`REFERENCES.md`'s own table of contents.
...
SKIP_DIRS = {".venv", ".git", "node_modules", "__pycache__", "site_src"}
LINK = re.compile(r"\[([^\]]*)\]\(([^)]+)\)")
