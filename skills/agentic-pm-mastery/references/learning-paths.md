...
| Capstone (required) | Proving the Agent core threads work together: one decision rebuilt from one trace id | The course whose `stage` is `Capstone`, after Agent core |
...
Recommend Agent core first unless the learner has already passed it, then the Capstone.
