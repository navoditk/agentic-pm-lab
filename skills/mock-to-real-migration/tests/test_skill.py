...
def test_migration_checklist_preserves_deferred_mocks():
    skill_text = (SKILL_DIR / "SKILL.md").read_text()

    assert "Preserve explicitly" in skill_text
    assert "Unit tests must not use a live network" in skill_text
