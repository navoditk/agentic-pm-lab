...
4. Explain why a caller claiming `RISK_USER` can read `PORT_B` here, what the specification says about self-reported metadata, and what would make identity authenticated for stdio and for HTTP.
...
2. "Put the user's role in `_meta` and have the server trust it." Explain that asserted identity is not authentication, and point to the build lab and the Authorization framework.
