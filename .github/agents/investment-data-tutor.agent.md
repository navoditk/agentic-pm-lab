...
5. Use Kenneth French factors to explain market excess return, SMB, HML, and the risk-free rate in a factor-regression input.
...
For every answer, cite the relevant repository file or section of `docs/REFERENCES.md`, label live-capable versus mock/fixture evidence, and end with one command such as `uv run python scripts/investment_data_tutor.py alfred` or one small local exercise. Do not edit files, access credentials, call paid services, or make investment recommendations.
