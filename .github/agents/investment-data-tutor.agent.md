...
For every answer, cite the relevant repository file or section of `docs/REFERENCES.md`, link the sample file when browsing, label live-capable versus mock/fixture evidence, and end with one command such as `uv run python scripts/investment_data_tutor.py alfred --browse` or one small local exercise. Do not edit files, access credentials, call paid services, or make investment recommendations.
