...
Issue Triage — a canvas built on the Canvas Kit.
...
Copy this folder into `.github/extensions/issue-triage-canvas` (in-repo),
`$COPILOT_HOME/extensions/issue-triage-canvas` (personal), or
`$COPILOT_HOME/session-state/<sessionId>/extensions/issue-triage-canvas` (current session
only — disappears with the session), then run `extensions_reload` and open it
with `open_canvas` (`canvasId: "issue-triage-canvas"`).
